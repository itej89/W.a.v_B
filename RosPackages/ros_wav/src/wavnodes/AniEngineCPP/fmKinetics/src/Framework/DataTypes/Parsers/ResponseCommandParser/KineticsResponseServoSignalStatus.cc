...
{
       KineticsResponseServoSignalStatus::KineticsResponseServoSignalStatus(string response) 
                    ...
                    if(RequestRecievedAck != NULL && RequestRecievedAck == KineticsResponseAcknowledgement::OK)
                    {
                        if(ResponseParts.size() == 3)
                        {
                           ...
                           if(ResponseErrorCondition == KineticsResponseAcknowledgement::NOP)
                            {
                                SignalState = ActuatorSignalStatusSymbols().ConvertFromString(ResponseParts.at(2));
                            }
