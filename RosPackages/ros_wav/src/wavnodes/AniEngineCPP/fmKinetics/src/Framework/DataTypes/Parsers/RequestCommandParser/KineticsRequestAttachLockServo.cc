#include "Framework/DataTypes/Parsers/RequestCommandParser/KineticsRequestAttachLockServo.h"

namespace fmKinetics
{

     KineticsRequestAttachLockServo::KineticsRequestAttachLockServo(Actuator::etype actuatorType)
     : KineticsRequestForActuator(CommandLabels::CommandTypes::LAT)
    {
        
        ActuatorType = actuatorType;
         vector<string> s {std::to_string(MachineConfig::Instance.MachineActuatorList.at(ActuatorType)->Address)};

        string Command = formCommand(s);
        
        Request = addDelimiters(Command);
    }
    
     KineticsRequestAttachLockServo::KineticsRequestAttachLockServo(string command)
     : KineticsRequestForActuator(CommandLabels::CommandTypes::LAT)
    {
        
        string _command = removeDelimiters(command);
        vector<string> contents = decomposeCommand( _command);
        
        if(contents.size() == 1)
        {
            int address = std::stoi(contents.at(0));
          
                ActuatorType = MachineConfig::Instance.getActuatorWith( address);
            
        }
    }
}